...
C
     ...
     1 four=4.d0,five=5.d0,NELEMENT=30.d0,NINPT=2.d0,NSVINT=7.d0,
     2 ELEMOFFSET=12861.d0)
c
...
c      beta=100            !指数软化的参数1
c      alpha=10  !指数软化的参数2
c      dlta_zero=1        !第二段软化的初始损伤点与第一段的比值
c      kink_point=5.3e-05          !转折点位移与最大位移的比值
c      dummy=0
c      multi=1                   !第二段软化的最大位移放大系数
...
C        ZETA is the fictitious viscosity used to regularize the instability problem. 
      ZETA=0.00005D0
c      stress(2)=stress(2)+ZETA*PROPS(4)*(one-d)*VU_loc(2)/dlta_o
c      ddsdde(2,2)=ddsdde(2,2)+ZETA*PROPS(4)*(one-d)/dlta_o/DTIME
      stress(1)=stress(1)+ZETA*PROPS(3)*(one-d)*VU_loc(1)/dlta_o
      ddsdde(1,1)=ddsdde(1,1)+ZETA*PROPS(3)*(one-d)/dlta_o/DTIME
...
c	
	...
	return
     ...
     4 DFGRD1, NOEL, NPT, LAYER, KSPT, KSTEP, KINC)
      INCLUDE 'ABA_PARAM.INC'
      CHARACTER*80 CMNAME
      PARAMETER(NINT=2.d0, NSTV=7.d0, NELEMENT=30.d0, ELEMOFFSET=12861.d0)
